...
# the hard-cutoff form, and the better of the two: `--window` changes the row set
# rather than the weights, so its rows cannot share the cache and the sweep costs
# more — but it also makes every later fit cheaper. Read the 25-origin
# confirmation, not the sweep, for the same reason as above.
$PY pooled3.py --model gbt --sets "" --leaves 15 --lr 0.03 --trees 300 --min-child 200 \
...
established as the leakage-free discipline, understates any parameter whose effect varies with
the origin — by 1.6x for the training window and 3.6x for the decay.
